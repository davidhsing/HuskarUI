...
indent | real | 18 | 缩进宽度
iconVisible | bool | false | 是否显示节点图标
defaultNodeIconSize | int | 16 | 默认节点图标大小
lineVisible | bool | false | 是否显示连接线
lineStyle | enum | HusTree.LineSolid | 连接线样式(来自 HusTree)
...
marginIcon | [HusMargin](internal://HusMargin) | { left: 2 } | 图标边距
marginText | [HusMargin](internal://HusMargin) | { left: 3 } | 文本边距
...
Column {
    ...
    HusTree {
        ...
        CodeBox {
            ...
            descTitle: qsTr('连接线')
            desc: qsTr(`
通过 \`lineVisible\` 设置是否显示连接线。\n
通过 \`lineStyle\` 设置连接线样式。\n
通过 \`lineWidth\` 设置连接线宽度。\n
通过 \`dashPattern\` 设置虚线模式(style == HusTree.LineDashed 时有效)。\n
...
Column {
    ...
    HusRadioBlock {
        ...
        initCheckedIndex: 0
        model: [
            { label: 'LineSolid', value: HusTree.LineSolid },
            { label: 'LineDashed', value: HusTree.LineDashed },
        ]
    }
    ...
    HusTree {
        id: showLineTreeView
        checkable: false
        iconVisible: showIconSwitch.checked
        lineVisible: showLineSwitch.checked
